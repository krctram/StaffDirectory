                        ...
                        listitem.forEach(function (li) {
                            ...
                            if (EndDateArr.length > 0) {
                                for (var i = 0; i < EndDateArr.length; i++) {
                                    ...
                                    fDateArr.map(function (b) { return b.Percentage ? endPercentage += parseInt(b.Percentage) : endPercentage += 0; });
...
var useravailabilityDetails = function () { return __awaiter(_this, void 0, void 0, function () {
    var EndDateArr, StartDateArr, modifiedArray, fDateArr, NextArr, StartArr, EndArr, availArr, WeekArr, FinalArr, NextArr1, StartArr1, EndArr1, availArr1, WeekArr1, FinalArr1, availTableHtml, EndDateArr, i_1, endPercentage, leastEndDate, checkFlag, k, j, i, Avainewday, Avainewday, EndDateArr1, i_2, endPercentage, leastEndDate, checkFlag, k, j, i, Avainewday, Avainewday, Poptions, Poptions;
    return __generator(this, function (_a) {
        EndDateArr = [];
        StartDateArr = [];
        modifiedArray = [];
        ...
        if (SelectedUserProfile[0].Availability != 0) {
            ...
            if (EndDateArr.length > 0) {
                ...
                availArr = [];
                WeekArr = [];
                for (i_1 = 0; i_1 < EndDateArr.length; i_1++) {
                    fDateArr = [];
                    endPercentage = 0;
                    leastEndDate = EndDateArr[i_1];
                    modifiedArray.forEach(function (cal) {
                        var edate = new Date(new Date(cal.EndDate).toLocaleDateString() + " 00:00");
                ...
                if (FinalArr.length > 0) {
                    ...
                    if (new Date(WeekArr[0]).getDay() == 0) {
                        Avainewday = new Date(new Date(WeekArr[0]).toLocaleDateString() + " 00:00");
                        Avainewday.setDate(Avainewday.getDate() + 1);
        ...
        if (SelectedUserProfile[0].Availability != 100 && FinalArr.length == 0 && WeekArr.length == 0) {
            EndDateArr1 = EndDateArr.sort(sortFunction);
            if (EndDateArr1.length > 0) {
                NextArr1 = [];
                StartArr1 = [];
                EndArr1 = [];
                availArr1 = [];
                for (i_2 = 0; i_2 < EndDateArr1.length; i_2++) {
                    fDateArr = [];
                    endPercentage = 0;
                    leastEndDate = EndDateArr1[i_2];
                    modifiedArray.forEach(function (cal) {
                        var edate = new Date(new Date(cal.EndDate).toLocaleDateString() + " 00:00");
                        var sdate = new Date(new Date(cal.StartDate).toLocaleDateString() + " 00:00");
                    });
                ...
                if (FinalArr1.length > 0) {
                    ...
                    if (new Date(WeekArr1[0]).getDay() == 0) {
                        Avainewday = new Date(new Date(WeekArr1[0]).toLocaleDateString() + " 00:00");
                        Avainewday.setDate(Avainewday.getDate() + 1);
